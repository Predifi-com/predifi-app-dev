...
export function LeaderboardSidebar({
  ...
  return (
    <div className="fixed left-0 top-[72px] h-[calc(100vh-72px)] w-[280px] border-r border-white/10 bg-[#0e1118]">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        !isSelected && !isHighlighted && 'border-white/8',
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs text-white/50">
        <div>
          Equity:{' '}
          <span className="font-mono text-white/70">${trader.totalEquity.toLocaleString()}</span>
        </div>
        <div>
          Vol: <span className="font-mono text-white/70">{formatVolume(trader.epochVolume)}</span>
        </div>
      </div>
...
function RankDisplay({ rank }: { rank: number }) {
  if (rank === 1) {
    return (
      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-yellow-500/20 text-xs font-bold text-yellow-400">
        🥇
      </div>
    )
  } else if (rank === 2) {
  ...
  return (
    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-white/5 text-xs font-bold text-white/70">
      {rank}
    </div>
  )
}
