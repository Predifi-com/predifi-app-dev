    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-foreground">{trader.name}</h3>
          {trader.isLive && <div className="h-2 w-2 animate-pulse rounded-full bg-green-400" />}
        </div>

        {trader.rankDelta !== 0 && (
          <div className={cn('flex items-center gap-0.5 text-xs font-semibold', trader.rankDelta > 0 ? 'text-green-400' : 'text-red-400')}>
      ...
      <div className="mb-3">
        <div className="flex items-baseline gap-2">
          <span className={cn('font-mono text-2xl font-semibold tabular-nums', trader.totalPnl >= 0 ? 'text-green-400' : 'text-red-400')}>
        ...
        <div className="mt-0.5 text-xs text-muted-foreground">
          Equity: <span className="font-mono text-foreground/70">${trader.totalEquity.toLocaleString()}</span>
        </div>
      </div>
      ...
      {densityMode === DensityMode.COMPACT ? (
        ...
        <DetailedExposure exposureData={exposureData} formatPrice={formatPrice} formatSize={formatSize} formatPnl={formatPnl} onPairClick={onPairClick} trader={trader} />
      )}
    </div>
  )
})
    ...
    <div className="space-y-1">
      {exposureData.map((data: any) => (
        <div key={data.pair} className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-2">
            {data.long > 0 && <span className="text-green-400">{data.long.toFixed(2)}L</span>}
            {data.short > 0 && <span className="text-red-400">{data.short.toFixed(2)}S</span>}
    ...
    <div className="space-y-1.5">
      {exposureData.map((data: any) => (
        <button
          ...
          className="w-full rounded p-1.5 text-left transition-colors hover:bg-accent/50"
        >
          ...
          <div className="mt-0.5 flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              {data.long > 0 && <span className="text-green-400">LONG {formatSize(data.long)}</span>}
              {data.short > 0 && <span className="text-red-400">SHORT {formatSize(data.short)}</span>}
              {data.long === 0 && data.short === 0 && <span className="text-muted-foreground/30">No position</span>}
            </div>
    ...
    <div className="space-y-2">
      {exposureData.map((data: any) => (
        <button
          key={data.pair}
          onClick={(e) => { e.stopPropagation(); onPairClick?.(data.pair) }}
          className="w-full rounded border border-border p-2 text-left transition-colors hover:border-border hover:bg-accent/50"
        >
          ...
          <div className="mt-1 space-y-0.5 text-xs">
            {data.long > 0 && (
              <div className="flex justify-between">
                ...
                <span className="font-mono text-muted-foreground">Avg: ${formatPrice(trader.exposure[data.pair].avgLongEntry)}</span>
              </div>
            )}
            {data.short > 0 && (
              <div className="flex justify-between">
                <span className="text-red-400">Short {formatSize(data.short)}</span>
                <span className="font-mono text-muted-foreground">Avg: ${formatPrice(trader.exposure[data.pair].avgShortEntry)}</span>
              </div>
            )}
            <div className="flex justify-between pt-0.5">
              <span className="text-muted-foreground">PnL:</span>
              <span className={cn('font-mono font-semibold', data.pnl >= 0 ? 'text-green-400' : 'text-red-400')}>{formatPnl(data.pnl)}</span>
            </div>
          </div>
        </button>
      ))}
